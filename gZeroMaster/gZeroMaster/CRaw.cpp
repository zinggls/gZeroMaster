...
{
	char buffer[3];
	while (CRawBase::ReadRegister(addr, 2, buffer, MAX_LOOP) != ERROR_SUCCESS) Sleep(10);	//Blocking 함수
...
{
	char buffer[3];
	LONG lLastError = CRawBase::ReadRegister(addr, 2, buffer, maxLoop);
...
void CRaw::OnChipConnect(CString chipModel)
{
	...
	if (chipModel == _T("B0")) {
		...
		m_regMap.insert(std::pair<CString, CReg>(_T("BIAS_REG9 [7:0]"), CReg(25, &m_strBiasReg9)));
