...
def zmq_server_init():
    context = util.zmq.Context(1)

    socket = context.socket(util.zmq.REP)
...
def zmq_server_start(socket):
    ...
    while True:
        ...
        json_obj = util.json.loads(json_msg.decode())
...
def serial_rx():
    global rx_thread_flag, comm_serial
    
    while rx_thread_flag:
        print(comm_serial.read())
